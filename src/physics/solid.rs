...
#[system(Update)]
fn render(mut grids: Query<(&mut Grid, &RenderLayers)>, mut meshes_and_materials: ResMut<MeshesAndMaterials>, asset_server: Res<AssetServer>, mut commands: Commands) {
    ...
    grids.iter_mut().for_each(|(mut grid, render_layers)| {
        grid.grid.iter_mut().enumerate().for_each(|(index, cell)| {
            ...
            // Will never panic, because the index will always be in the grid.
            let translation = grid.index_to_translation(index).unwrap();
                    ...
                    .contains_key(&colour_not_nan)
                {
                    meshes_and_materials
                        .colour_materials
                        .insert(colour_not_nan, asset_server.add(ColorMaterial::from_color(solid.colour)));
